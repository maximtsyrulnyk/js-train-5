...
function sumArray(array) {
  let totalSum = 0; // Ініціалізуємо змінну для зберігання суми
  // Проходимося по всіх індексах масиву
  for (let index in array) {
    // Додаємо кожен елемент до загальної суми
    totalSum += array[index];
  }
  return totalSum; // Повертаємо суму
}
...
function multiplyArray(array) {
  let totalProduct = 1; // Ініціалізуємо змінну для зберігання добутку
...
function printStrings(array) {
  ...
  // Повторюємо рядок двічі та виводимо його на консоль
  const result = arrayAsString + arrayAsString;
  console.log(result);
}
...
function concatenateStrings(array, subString) {
  ...
  const containsSubstring = arrayAsString.includes(subString);
  // Виводимо результат
...
// Ініціалізуємо дві змінні (max і min) з першим елементом масиву.
let max = array[0];
let min = array[0];

// Ініціалізуємо дві змінні (maxIndex і minIndex) з нульового значення.
let maxIndex = 0;
let minIndex = 0;

// Використовуємо цикл for для ітерації по масиву.
for (let i = 0; i < array.length; i++) {
  // Перевіряємо, чи є поточний елемент масиву більшим за поточне максимальне значення.
  if (array[i] > max) {
    // Якщо це так, оновлюємо max з поточним елементом масиву та maxIndex з поточним індексом.
    max = array[i];
    maxIndex = i;
  }
  // Перевіряємо, чи є поточний елемент масиву меншим за поточне мінімальне значення.
  else if (array[i] < min) {
    // Якщо це так, оновлюємо min з поточним елементом масиву та minIndex з поточним індексом.
    min = array[i];
    minIndex = i;
  }
}
